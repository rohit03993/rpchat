...
function requestPayDiscount({ userId, note }) {
  ...
  // Mark funnel first, then support message (own write)
  if (!db.payFunnels[id]) {
    db.payFunnels[id] = {
      userId: id,
      stage: stage || "abandon",
  ...
  // Auto-shoot QR offer for the pack they left (match by id, then by hours)
  let winback = null;
  try {
    const dbAfter = readDb();
    const funnelAfter = dbAfter.payFunnels && dbAfter.payFunnels[id];
    let packId =
      (funnelAfter &&
        (funnelAfter.leadPackageId || funnelAfter.packageId)) ||
      "";
    packId = String(packId || "").trim();
    let pack = packId ? getPackage(packId) : null;
    if (!pack && funnelAfter && funnelAfter.hours != null) {
      const wantH = Number(funnelAfter.hours);
      pack =
    ...
    winback = grantWinbackOffer(id, {
      packageId: packId || undefined,
      force: true,
      allowWithTime: true,
      source: "discount_ask",
    });
    // Persist skip reason for admin debugging
    if (winback && (winback.skipped || winback.ok === false)) {
      const dbR = readDb();
      if (dbR.payFunnels && dbR.payFunnels[id]) {
        dbR.payFunnels[id].lastOfferSkip =
          winback.reason || winback.error || "unknown";
        dbR.payFunnels[id].updatedAt = Date.now();
        writeDb(dbR);
      }
    }
  } catch (e) {
    winback = {
      ok: false,
      skipped: true,
      reason: (e && e.message) || "offer_failed",
    };
  }
  ...
  return Object.values(db.users || {})
    .map((u) => {
      ...
      return {
        ...
        accessExpiresAt:
          Number(u.accessExpiresAt) > now
            ? Number(u.accessExpiresAt)
            : hoursLive > 0.0001
              ? Math.round(now + hoursLive * 3600000)
              : 0,
        hasPaid: !!u.hasPaid,
...
function grantWinbackOffer(userId, opts) {
  ...
  const reasonLine =
    options.source === "discount_ask"
      ? "Special offer for you (discount request).\n"
      : "Special offer — continue your chat.\n";

  const text =
    reasonLine +
    "Pay ₹" +
    price +
    (listPrice > price ? " (was ₹" + listPrice + ")" : "") +
    " for " +
    (pack.label || pack.id) +
    ".\n" +
    "Scan the QR below. UPI note / remark = " +
    id +
    " (your User ID).\n" +
    "After payment lands, access unlocks automatically.";
