...
function submitPayment({ userId, packageId, screenshotBase64, upiNote, utr }) {
 ...
 * Match order: UTR → unique amount+screenshot → else needs_review / no_match.
 */
function ingestSmsCredit({ smsText, amountInr, utr }) {
  ...
  if (decision.action === "approve" && decision.paymentId) {
    ...
    if (!dbAfter.smsCredits || typeof dbAfter.smsCredits !== "object") {
      dbAfter.smsCredits = {};
    }
    if (!approved.ok) {
      creditRow.action = "needs_review";
      creditRow.reason = approved.error || "Approve failed";
      dbAfter.smsCredits[creditId] = creditRow;
      writeDb(dbAfter);
      return {
        ok: false,
        action: "needs_review",
  ...
  db.smsCredits[creditId] = creditRow;
  writeDb(db);
  return {
    ok: true,
    action: decision.action,
...
function listUsers() {
  ...
  return Object.values(db.users || {})
    .map((u) => {
      ...
      const noticeInfo = noticeStatsForUser(db, u.userId);
      return {
        userId: u.userId,
        pin: u.pinPlain || null,
        isLegacy,
        ...
        noticeUnread: noticeInfo.noticeUnread,
        lastNoticeSeen: noticeInfo.lastNoticeSeen,
        lastNoticeAt: noticeInfo.lastNoticeAt,
      };
    })
...
function publicSupportThread(thread) {
  if (!thread) {
    return { userId: "", status: "open", updatedAt: null, messages: [] };
  }
  return {
...
function addSupportMessage({ userId, from, text, screenshotBase64 }) {
  ...
  thread.status = "open";
  if (role === "user") thread.needsAdmin = true;
  if (role === "admin") thread.needsAdmin = false;
  writeDb(db);
