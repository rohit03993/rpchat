...
function wipeChatInDb(db, userId) {
  if (db.chats && db.chats[userId]) {
    ...
    db.chatArchive[userId] = Object.assign({}, db.chats[userId], {
      archivedAt: Date.now(),
    });
...
function tickUserHours(userId) {
  ...
  if (Number(user.hoursBalance || 0) <= 0.0001) {
    ...
    return {
      ok: false,
      error:
        "Time/plan khatam. Chat history clear ho gayi. Naya package lo, phir naya chat start karo.",
...
function listUsers() {
  ...
  const archive = db.chatArchive || {};
  return Object.values(db.users || {})
    .map((u) => {
      const mine = payments.filter((p) => p.userId === u.userId);
      const isLegacy = !/^\d{4}$/.test(String(u.userId || ""));
      const live = chats[u.userId];
      const archived = archive[u.userId];
      const session = live || archived || null;
      const history = (session && session.history) || [];
      const visibleMsgs = history.filter(
        (m) => m && m.content && !/^Setup locked for this chat:/i.test(m.content)
      );
      const charName =
      ...
      return {
        ...
        chatMsgCount: visibleMsgs.length,
        chatLive: !!live,
        chatArchived: !live && !!archived,
...
/** Admin can read live or archived chat (users lose live copy when plan ends). */
function getChatSessionAdmin(userId) {
  const db = readDb();
  const id = String(userId || "").trim();
  if (db.chats && db.chats[id]) {
    return { session: db.chats[id], source: "live" };
  }
  if (db.chatArchive && db.chatArchive[id]) {
    return { session: db.chatArchive[id], source: "archived" };
  }
...
function adminSetHours(userId, hours) {
  ...
  writeDb(db);
  return { ok: true, user: publicUser(user), chatCleared: val <= 0 };
}
...
function saveChatSession(userId, session) {
  ...
  if (Number(user.hoursBalance || 0) <= 0.0001) {
    ...
    return {
      ok: false,
      error: "No time left — chat history cleared with plan end.",
      chatCleared: true,
    };
  }
  ...
  db.chats[userId] = {
    updatedAt: Date.now(),
    // Soft expiry hint: roughly when current balance would run out if used continuously
    expiresAt: Date.now() + Math.ceil(hoursLeft * 3600000),
...
function getChatSession(userId) {
  const db = readDb();
  const user = db.users[userId];
  if (!user || Number(user.hoursBalance || 0) <= 0.0001) {
    if (db.chats && db.chats[userId]) {
      wipeChatInDb(db, userId);
      writeDb(db);
    }
  ...
  // If balance already gone somehow, wipe
  if (session.expiresAt && Date.now() > session.expiresAt && Number(user.hoursBalance || 0) <= 0.0001) {
    wipeChatInDb(db, userId);
    writeDb(db);
    return null;
  }
