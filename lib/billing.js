...
function requestPayDiscount({ userId, note }) {
  ...
  return {
    ok: true,
    funnel: publicPayFunnel(db.payFunnels[id]),
...
function adminGetSettings() {
  ...
  return {
    ...
    winbackPackageId: s.winbackPackageId || "day",
    winbackPriceInr: Math.round(Number(s.winbackPriceInr) || 50),
...
function ingestSmsCredit({ smsText, amountInr, utr }) {
  ...
  const s = getSettings();
  if (s.winbackEnabled && Number(s.winbackPriceInr) > 0) {
...
/**
 * Trial/time-over unpaid: grant win-back offer, Support QR message, SMS pay-intent.
 * Idempotent within WINBACK_COOLDOWN_MS.
 */
function grantWinbackOffer(userId) {
  const id = String(userId || "").trim();
  if (!id) return { ok: false, error: "Login required" };
  ...
  const pack = getPackage(s.winbackPackageId || "day");
  const price = Math.round(Number(s.winbackPriceInr));
  ...
  if (
    ...
    // Refresh intent window if still within cooldown
    if (!db.payIntents || typeof db.payIntents !== "object") db.payIntents = {};
    db.payIntents[id] = {
      userId: id,
      packageId: pack.id,
  ...
  writeDb(db); // persist hour sync before support msg (separate write)
