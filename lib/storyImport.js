...
async function fetchHtml(url) {
  ...
  try {
    const res = await fetch(url, {
      ...
      signal: ctrl.signal,
      headers: {
        // Look like a normal Chrome phone — many story sites block bot UAs
        "User-Agent":
    ...
    if (!res.ok) {
      ...
      if (code === 403 || code === 401 || code === 429) {
        return {
          ok: false,
          error:
            "Site blocked auto-read (" +
            code +
            "). Use Paste text — copy story from browser.",
          code: "SITE_BLOCKED",
          status: code,
        };
      }
      if (code === 404) {
        return { ok: false, error: "Story page not found (404)", status: code };
      }
      return {
        ok: false,
        error: "Could not open story page (" + code + ")",
        status: code,
      };
    }
    ...
    // Cloudflare / challenge pages often 200 with tiny challenge HTML
    if (
      /cf-browser-verification|just a moment|attention required|access denied|captcha/i.test(
        html.slice(0, 4000)
      ) &&
      html.length < 12000
    ) {
      return {
        ok: false,
        error:
          "Site blocked auto-read (challenge). Use Paste text — copy story from browser.",
        code: "SITE_BLOCKED",
      };
    }
...
async function importStoryFromUrl(startUrl) {
  ...
  if (!pages.length) {
    return {
      ...
      error:
        lastError ||
        "Could not read story text from that URL — try Paste text",
      code:
        lastError && /blocked|403|401|429|challenge/i.test(lastError)
          ? "SITE_BLOCKED"
          : "IMPORT_FAILED",
    };
  }
